 ...
 * @returns 
 */
function SelectedTokenList({ selectedTokens }) {
    const [ selectedTokenFirstItem, setSelectedTokenFirstItem ] = useState(null)
    const { data: selectedItemInfo } = useSWR(`https://api.cryptoverse.vip/land/${selectedTokenFirstItem}`);

    useEffect(() => {
        document.addEventListener("selectedToMint", function (e) { 
            if(e.detail.tokens.length) {
                 let url = `${e.detail.tokens[e.detail.tokens.length-1].paddedTokenId}`;
                 setSelectedTokenFirstItem(url);
            }
         });
    })
    ...
    return (
        <>
           ...
           {selectedTokens.length > 0 &&
                <div className="scale-[0.85] [ 2xl:scale-[1] origin-bottom-left rounded-tl-xl rounded-br-xl transition duration-700 shadow-xl shadow-torch-red/20 bg-haiti-400/100 shadow-2xl space-x-4 fixed bottom-0 mb-14 ml-4 z-[10] space-y-1 text-white left-0 flex w-[500px] bg-haiti-400 rounded-lg p-6">
                    ...
                    <div className={`space-y-1 w-1/2`}>
                        <span className="text-sm flex justify-between space-x-2 border-b border-solid border-white/20 pb-1"><span>TokenId:</span> {selectedTokens[selectedTokens.length-1]?.paddedTokenId}</span>
