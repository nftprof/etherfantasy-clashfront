...
import MintTokensWidget from "@ui/landSelector/MintTokensWidget";
import useSWR from "swr";

import LandSelectorTool from "@vendors/LandSelector"
import SearchForToken from './SearchForToken';
import SelectedTokenList from './SelectedTokenList';
import TokenLegend from './TokenLegend';
import Help from './Help';

import ClipLoader from "react-spinners/ClipLoader";
import MintingStatus from "@ui/landSelector/MintingStatus";
 ...
 * @constructor
 */
function WebGL({ balances, callback }) {
    ...
    const { data: soldItemsL3 } = useSWR(() => selectedL2EnabledItem ? `https://api.cryptoverse.vip/land/minted/l3/${selectedL2EnabledItem}` : null, { refreshInterval: 5000 });
    ...
    useEffect(() => {
        ...
        if(scene === null && canvas?.length === 0) {
            const internalScene = new LandSelectorTool({
    ...
    useEffect(() => {
        if(scene !== null && dataLoaded && soldItems) {
            ...
            scene.updateSoldLandItems();
            scene.setUserBalances(balances);
        }
    ...
    return (
        <>
            ...
            {/*
                Minting status
                occurs once a user starts the minting process
                the step by step helps the user know the progress
            */}
            {mintingStatus &&
                <MintingStatus
                    callBack={() => setMintingStatus(false)}
            ...
            */}
            ...
            {selectedTokens.length > 0 && mintingStatus === false &&
                <MintTokensWidget
                    selectedTokens={selectedTokens}
                    resetSelectedTokens={(e) => {
                        selectedTokens.map((child) => {
                            scene.addToSelectedArray(child);
                        })
                    }}
                    mintSelectedTokens={mintSelectedTokens}
                    removeSelectedToken={(e) => {
                        scene.addToSelectedArray(e);
                    }}
                />
            }
