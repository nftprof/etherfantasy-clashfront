/* ============================================================================
   EF_BATTLEFIELD — the MOBA's Layer-1 scene builder, factored out of game state.
   ----------------------------------------------------------------------------
   ...
   Usage (browser, three.js r128):
     ...
     <script src="shared/ef_battlefield.js"></script>
   ...
   Renders the nine layers (see docs/briefs/MAP-AUTHORING-GUIDE.md):
     ...
     5 fog + matching background + the light rig
...
function buildBattlefield(scene, opts){
  ...
  /* tower / core markers — in a live match these arrive as net units; for the DESIGNER
     preview we stamp simple pedestals so the parcel reads complete. */
  (M.towers||[]).forEach(function(t){
    var blue = !/def|red|r$/i.test(String(t.side||''));
    var col = blue ? 0x4aa3ff : 0xff5d5d;
    var y = heightAt(t.x, t.z);
    ...
    ped.position.set(t.x, y+1.0, t.z); root.add(ped);
    ...
    shaft.position.set(t.x, y+2.0+4.5, t.z); root.add(shaft);
    ...
    cap.position.set(t.x, y+2.0+9+1.3, t.z); root.add(cap);
  });
...
window.EF_BATTLEFIELD = { buildBattlefield: buildBattlefield, TUNED: TUNED, VERSION: '1' };
})();
