...
//   • BLACK LAKES = `rivers[]` with **`fill: true`** (worldfield.js FILL water, 2026-07-11 —
//     supersedes the clamped-band v1): the Lake That Watches (at the W gate), the Lantern-dark
//     (W gallery), the MERE OF DOMINUS (a RING band: the Bastion island sits dry inside it),
//     the Pale Cenotes (a field of six round pools), the Ferry Dark (E), the Sunken Court +
//     the Drowned Meadows (S), and the five Banquet hall channels — each windows into battle
//     maps at its HONEST full width (a Mere-interior parcel is genuinely underwater; the carve/
//     repair machinery guarantees a causeway). The three connecting channels stay band-clamped
//     linear flows.
...
function buildField() {
  ...
  const out = {
    _meta: {
      ...
      widths: "zone-units at zone scale (1 parcel ≈ 0.65 u across); world-units at battle scale are derived per-parcel by worldfield.js (tier caps + floors). LAKES carry fill: true — the zoneCap clamp is BYPASSED and the full authored width windows into battle maps (true water geometry at battle scale); playability on water-dominant parcels is guaranteed by the generator's carve + validateAndRepair causeway machinery instead. Channels/veins stay band-clamped linear flows.",
      ...
      water: "rivers[] = BLACK LAKES + flooded channels. Lakes + cenotes + the five Drowned-Banquet halls carry `fill: true` (worldfield.js FILL water): the full authored width windows into battle maps — the Lake That Watches (Blackmere's Gate threshold), the Lantern-dark, the MERE OF DOMINUS (a RING band — the Bastion island is the dry inside, the Bastion Causeway its one bridge), the six Pale Cenotes, the Ferry Dark, the Sunken Court, the Drowned Meadows. The Longdark/Weeping/Bier channels are band-clamped linear flows. Causeway crossings (roads over the water) are the chokepoints; on a water-dominant parcel the generator's repair carve IS the causeway. Two SE fissures carry `magma: true` (additive; palette = the registry VOLCANIC family — UW3's fire creeping up).",
    ...
    zone: "UW2",
    rivers: [
      // fill: true = TRUE LAKES (worldfield.js FILL water, 2026-07-11): the entry's full authored
      // width windows into battle maps honestly (no zoneCap clamp) — black lakes read as lakes,
      // the Mere ring as a real ring-mere around the dry Bastion island. LINEAR flows (the three
      // connecting channels + the two SE magma veins) stay band-clamped: they are flows, not lakes.
      { id: "UW2-LK-MERE", name: "The Mere of Dominus", width: 4.5, fill: true, pts: lakeMere },
      { id: "UW2-LK-VIGIL", name: "The Lake That Watches", width: 5.0, fill: true, pts: lakeVigil },
      { id: "UW2-LK-LANTERN", name: "The Lantern-dark", width: 5.5, fill: true, pts: lakeLantern },
      { id: "UW2-LK-FERRY", name: "The Ferry Dark", width: 5.0, fill: true, pts: lakeFerry },
      { id: "UW2-LK-COURT", name: "The Sunken Court", width: 5.0, fill: true, pts: lakeCourt },
      { id: "UW2-LK-MEADOW", name: "The Drowned Meadows", width: 5.5, fill: true, pts: lakeMeadow },
      ...cenotes.map((c, i) => ({ id: `UW2-LK-CENOTE${i + 1}`, name: `The Pale Cenotes (${i + 1})`, width: 2.8, fill: true, pts: c })),
      ...
      ...halls.map((h, i) => ({ id: `UW2-HALL${i + 1}`, name: HALL_NAMES[i], width: 1.1, fill: true, joins: "UW2-LK-MERE", pts: h })),
