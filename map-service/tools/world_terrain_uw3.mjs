...
//   • THE MAGMA THRONE: the central caldera lava pool — a `rivers[]` disc with `magma: true`
//     AND **`fill: true`** (worldfield.js FILL water, 2026-07-11): its full authored width
//     windows into battle maps as a SOLID LAVA LAKE, not a ring stripe (palette = the registry
//     VOLCANIC family, whose battle-map WATER cells already render as lava).
...
function buildField() {
  ...
  const CASTLES = [
    // STORY OVERRIDE (owner 2026-07-27): the Vault-Palace is the game's finale (EP7 Castle of Shadows)
    // and MUST read as the top rank — PALACE (3-ring) — even though UW3 has no EPIC (the scaled ladder
    // would make it a 2-ring CASTLE). Ring-count = visible rank; the final vault gets 3. It sits on the
    // best LARGE (heroParcels quota stays LARGE=3); only the castle GEOMETRY is promoted to a palace.
    { id: "UW3-PALACE-VAULTPALACE", kind: "PALACE", at: VP_AT.slice(), townEstateId: vaultPalace.parcelId,
      name: "The Vault-Palace of Luxuria", ref: "the Throne precinct's palace behind/above the Magma Throne (seen from the Penitents' Way) — the Inferno Champion's seat and the FINAL VAULT's warding palace; STORY-DESIGNATED PALACE (3-ring, owner 2026-07-27): the finale reads as the world's top rank despite riding a LARGE (no GIANT/EPIC exists in the vault — the §3c ladder scaled, the isles precedent)" },
  ...
  const out = {
    _meta: {
      ...
      magma: "rivers[] are ALL MAGMA here (`magma: true` on every entry — additive flag; palette = the registry VOLCANIC family, whose battle-map WATER cells already render as lava): the MAGMA THRONE (the central caldera — `fill: true`, worldfield.js FILL water: it windows into battle maps at its honest full width, a SOLID lava lake, the final boss's seat and the deepest point of the world) + four MAGMA FLOWS radiating N/E/S/W between the boulevards + the SE fissure vein (linear flows, band-clamped). The Ring of Sighs crosses the four flows on ember bridges (road-over-magma = the crossing chokepoints when the real-water/hazard phase lands, CONTINUOUS-WORLD-TERRAIN §4b); the generator's carve/repair machinery guarantees a crossing on throne-flooded parcels.",
      ...
      castles: "castles[] per CONTINUOUS-WORLD-TERRAIN §3c with the ladder SCALED to what exists (NO GIANT/EPIC in the vault — the isles precedent): the TOP fortification is the Vault-Palace of Luxuria (" + vaultPalace.parcelId + ", the playable LARGE nearest the throne — the Inferno Champion's warding seat at the FINAL VAULT), emitted as a STORY-DESIGNATED PALACE (3-ring, owner 2026-07-27) so the game's finale reads as the world's top rank even though it rides a LARGE — a deliberate override of the scaled ladder for the single most important location; the four SIN-PALACE compounds are KEEPs on the remaining playable LARGEs nearest the palace ring (" + sinKeeps.map((p) => p.parcelId).join("/") + "). All five castle estates are L3-subdivided — every UW3 castle is fully designated (LARGE 3 heroParcels each); castles grow WALL/GATE/TOWER rings via maps/generate.js castleLayout.",
    ...
    zone: "UW3",
    rivers: [
      // fill: true = the caldera LAVA LAKE (worldfield.js FILL water, 2026-07-11): the Throne's
      // full authored width windows into battle maps honestly — a solid ~4.7-radius lava disc,
      // not a ring stripe. The four radiating flows + the SE vein are LINEAR flows: band-clamped.
      { id: "UW3-THRONE", name: "The Magma Throne", width: 5.0, magma: true, fill: true, pts: throne },
