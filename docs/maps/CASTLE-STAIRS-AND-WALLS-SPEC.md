...
| **R-GAP** | Multi-ring wards stay ROOMY at EVERY point — walls never merge (owner 2026-07-28). | v16: inner rings derive from the previous ring **per anchor** with `WARD_MIN = 10u` centerline clearance (uniform scaling merged walls at dented spots); `gapIn` = the ACTUAL min ward clearance; anchors that would exit the parcel polygon pull further inward. Sweep asserts ≥8u at every anchor pair. |
| **R-JOINT** | Wall runs join SEAMLESSLY — no slit at a bend or at a tower (owner 2026-07-28: "outer wall got gaps"). | Render kit: a corner post drum at EVERY ring anchor seals each wall↔wall / wall↔tower miter (gate anchors framed by the gatehouse). |
