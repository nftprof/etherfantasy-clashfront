...
| **R-GAP** | Wards stay roomy at EVERY point: ≥12u wall-centerline gap absolute (4.2 wall + 3.4 stair + margin), target 16u; segment-level, not just anchors. Too tight ⇒ fewer rings (R-RING), never thinner wards. | `gapIn` = actual min | G+CI |
...
| **R-PAD** | No-overlap build pads: baked tower/CC pads sit off the wall band (≥8.8u), clearly off any castle structure (≥13.8u) and the keep. | `PAD_WALL/PAD_STRUCT` | G |
