...
// PALACE ESTATE battle maps (canon decisions 4/5/18; 2026-07-11) — pre-designed maps for the five
// unsubdivided-EPIC palace estates, committed KEYED BY THE ESTATE's OWN parcelId (the loader keys
// by id string, so estate ids work verbatim). Generated by map-service/tools/estate_palace_maps.mjs;
// the world fields mark each PALACE castles[] entry with estateMapId = this key.
const PALACE_ESTATES: [string, string][] = [
  ['1101100', 'The Bastion of Dominus (UW2)'],
  ['1031491', 'The Palace of Masks (ENT)'],
  ['1020371', 'The Grand Academy (EDU)'],
  ['1071732', 'The Vermilion Palace / Zhongdu (HUB)'],
  ['1001178', 'The Grand Exchange (BUS)'],
];

for (const [id, name] of PALACE_ESTATES) {
  test(`palace estate map ${id} (${name}) loads by ESTATE id + passes all five invariants`, () => {
    const bf = loadParcelBattlefield(id);
    assert.ok(bf, `loadParcelBattlefield(${id}) returned the estate map`);
    assert.equal(bf.meta?.parcelId, id, 'keyed to the ESTATE parcelId');
    assert.equal(bf.meta?.sizeClass, 'EPIC', 'declares the estate sizeClass');
    const v = validateBattlefield(bf);
    assert.ok(v.ok, `validateBattlefield failed for ${id}: ${JSON.stringify(v)}`);
    assert.equal((bf.structures ?? []).filter((s) => s.kind === 'CORE').length, 2, 'CORE per side');
    assert.ok(
      (bf.structures ?? []).some((s) => s.anchorId.startsWith('castle_')),
      'carries the palace castle ring (castle_* anchors)'
    );
  });
}
